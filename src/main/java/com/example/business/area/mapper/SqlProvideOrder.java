...
//        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String sql = " select * from `order`" +
                ...
                "where 1=1 ";
        if (request.getArea() != null && !Objects.equals(request.getArea(), "")) {
            sql += " and area.name like '%" + request.getArea() + "%' ";
        }
        if (request.getUser() != null && !Objects.equals(request.getUser(), "")) {
            sql += " and user.username like '%" + request.getUser() + "%' ";
        }
        if (request.getTime() != null) {
            sql += " and order.time = '" + request.getTime() + "' ";
        }
        if (request.getDay() != null && !Objects.equals(request.getDay(), "")) {
